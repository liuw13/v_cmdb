...
device_white = ['eth0', 'eth1', 'eth2', 'eth3', 'bond0', 'bond1','ens33']
...
def get_meminfo():
    with open("/proc/meminfo") as f:
        tmp = format(int(f.readline().split()[1]) /1024/1024, '.2f')
        return tmp
...
def get_device_info():
    ...
    for device, device_info in psutil.net_if_addrs().items():
        if device in device_white:
            tmp_device = {}
            for sincaddr in device_info:
                if sincaddr.family == 2:
                    tmp_device['ip'] = sincaddr.address
                if sincaddr.family == 17:
                    tmp_device['mac'] = sincaddr.address
...
def send(data):
    url = 'http://192.168.40.128:8001/cmdb/api_servers/'
